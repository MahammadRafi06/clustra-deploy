...
export function useModelDetail(id: string | null) {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    api.getModel(id).then(setData).catch(() => {}).finally(() => setIsLoading(false));
  }, [id]);
...
export function useDownloadModel() {
  ...
  const mutate = useCallback((req: DownloadRequest, opts?: { onSuccess?: () => void }) => {
...
export function useSoftDelete() {
  ...
  const mutate = useCallback((id: string) => {
    setIsPending(true);
    api.softDeleteModel(id).catch(() => {}).finally(() => setIsPending(false));
  }, []);
...
export function useHardDelete() {
  ...
  const mutate = useCallback((id: string, opts?: { onSuccess?: () => void }) => {
    setIsPending(true);
    api.hardDeleteModel(id).then(() => opts?.onSuccess?.()).catch(() => {}).finally(() => setIsPending(false));
  }, []);
  return { mutate, isPending };
}

export function useRestoreModel() {
  const mutate = useCallback((id: string) => { api.restoreModel(id).catch(() => {}); }, []);
  return { mutate };
}
...
export function useBulkAction() {
  const mutate = useCallback((req: BulkActionRequest) => { api.bulkAction(req).catch(() => {}); }, []);
  return { mutate };
}
