...
export const JobsPanel: React.FC<{visible: boolean; onClose: () => void}> = ({visible, onClose}) => {
    ...
    const {data, isLoading, error, isPollingPaused, refetch} = useJobs({kind: kindFilter || undefined, status: statusFilter || undefined});
    ...
    return (
        <>
            <SlidingPanel hasCloseButton={true} hasNoPadding={true} header={<strong>Jobs</strong>} isNarrow={true} isShown={visible} onClose={onClose}>
                <div role='dialog' aria-modal='true' aria-label='Jobs'>
                    ...
                    <div className='model-cache__drawer-body'>
                        {error && <ErrorBanner message={`Failed to load jobs${isPollingPaused ? '; polling paused' : ''}: ${error.message}`} onRetry={refetch} />}
                        {cancelJob.error && <ErrorBanner message={`Failed to cancel job: ${cancelJob.error.message}`} />}
                        {retryJob.error && <ErrorBanner message={`Failed to retry job: ${retryJob.error.message}`} />}
                        {isLoading ? (
