...
export function ContextSelector({ value, onChange }: ContextSelectorProps) {
  ...
  return (
    <div className="cext-context-panel">
      <div className="cext-context-panel__header">
        <div className="cext-context-panel__title">Target Context</div>
        <div className="cext-context-panel__subtitle">
          Choose the Argo CD project and application that should receive the generated manifest.
        </div>
      </div>

      <div className="cext-context-grid">
        <div className="cext-field">
          <label htmlFor="cext-project-select">Project</label>
          <div className="cext-argo-select">
            <Select
              id="cext-project-select"
              value={projectName}
              options={[
          ...
          {!projectsError && (
            <p className="cext-field-hint">
              Existing Argo CD projects visible to the current user.
            </p>
          )}
        </div>

        <div className="cext-field">
          <label htmlFor="cext-application-select">Application</label>
          <div className="cext-argo-select">
            <Select
              id="cext-application-select"
          ...
          {!applicationsError && (
            <p className="cext-field-hint">
              Existing Argo CD applications inside the selected project.
            </p>
          )}
        </div>
      </div>
      ...
      {!applicationsLoading && projectName && applications.length === 0 && !applicationsError && (
        <NoticeAlert
          variant="warning"
          message={`No applications are currently available in project "${projectName}".`}
        />
      )}
      ...
      {!value && projectName && applications.length > 1 && !applicationsLoading && !applicationsError && (
        <NoticeAlert
          variant="info"
          message="Pick an application to load deploy context, job history, and Git target details."
        />
      )}

      {value && (
        <div className="cext-context-summary">
          <div className="cext-context-summary__title">Selected Target</div>
          <div className="cext-context-summary__row">
            <span className="cext-context-summary__label">Application</span>
            <span>{value.appNamespace}/{value.appName}</span>
          </div>
          <div className="cext-context-summary__row">
            <span className="cext-context-summary__label">Project</span>
            <span>{value.projectName}</span>
          </div>
          {selectedSource?.repoURL && (
            <div className="cext-context-summary__row">
              <span className="cext-context-summary__label">Repository</span>
              <span>{selectedSource.repoURL}</span>
            </div>
          )}
          {selectedSource?.path && (
            <div className="cext-context-summary__row">
              <span className="cext-context-summary__label">Path</span>
              <span>{selectedSource.path}</span>
            </div>
          )}
          {selectedSource?.targetRevision && (
            <div className="cext-context-summary__row">
              <span className="cext-context-summary__label">Revision</span>
