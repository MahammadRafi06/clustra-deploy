  ...
  return (
    <div className="cext-form">
      ...
      <div className="cext-field">
        ...
        <div className="cext-radio-group">
          <label className="cext-radio-label">
      ...
      {inputMode === 'yaml_path' ? (
        <FieldInput
          def={{ key: 'yaml_path', label: 'YAML Path (server-side)', type: 'text', required: true, placeholder: '/app/output/my-experiment.yaml', hint: 'Must be within AICONF_OUTPUT_BASE_DIR' }}
        ...
        <FieldInput
          def={{ key: 'config', label: 'Inline Config (JSON)', type: 'textarea', required: true, rows: 8, placeholder: '{\n  "model_path": "...",\n  ...\n}' }}
          value={values.config ?? ''}
          error={errors.config}
          onChange={setValue}
        />
      ...
      {/* Advanced */}
      <AdvancedSection>
        <FieldInput def={{ key: 'top_n', label: 'Top-N Results', type: 'number', min: 1, max: 50, placeholder: '5' }} value={values.top_n ?? ''} error={errors.top_n} onChange={setValue} />
      </AdvancedSection>

      {/* Deploy section */}
      <div className="cext-section-divider"><span>Deploy</span></div>
      <FieldInput
        def={{ key: 'mode', label: 'Deployment Mode', type: 'select', required: true, options: DEPLOY_MODE_OPTIONS, includeEmptyOption: false, hint: 'Chooses the manifest layout committed to your ArgoCD app repo.' }}
        value={values.mode ?? ''}
        error={errors.mode}
        onChange={setValue}
      />
